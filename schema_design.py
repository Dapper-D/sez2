...
def ensure_schema():
    ...
    """)
    # --- Add retention policy for 7 days on all hypertables ---
    cur.execute("SELECT add_retention_policy('candle_data_1min', INTERVAL '7 days', if_not_exists => TRUE);")
    cur.execute("SELECT add_retention_policy('candle_data_15sec', INTERVAL '7 days', if_not_exists => TRUE);")
    cur.execute("SELECT add_retention_policy('technical_indicators', INTERVAL '7 days', if_not_exists => TRUE);")
    cur.execute("SELECT add_retention_policy('trading_signals', INTERVAL '7 days', if_not_exists => TRUE);")
    conn.commit()
    cur.close()
    conn.close()
